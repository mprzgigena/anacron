        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
          <div>
            ...
            <input
              ...
              class="input-field mt-1"
            />
            <p v-if="validationErrors.fechaTurno" class="mt-1 text-sm text-red-600">
              {{ validationErrors.fechaTurno }}
            </p>
          </div>
  ...
  setup(props) {
    ...
    const profesionalesFiltrados = computed(() => {
      ...
      return profesionales.value.filter(profesional => 
        profesional.profesionalEspecialidades?.some(pe => 
          pe.especialidadId == form.especialidadId
        )
      )
    })
    ...
    watch(() => form.profesionalId, () => {
      ...
      // Asignar automáticamente el establecimiento del profesional seleccionado
      if (form.profesionalId) {
        const profesional = profesionales.value.find(p => p.id == form.profesionalId)
        if (profesional) {
          // Usar establecimientoId directo del profesional
          form.establecimientoId = profesional.establecimientoId
        }
      } else {
